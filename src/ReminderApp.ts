...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    ...
    if (this._remindersHandler.size() === 0) {
      ...
      const result = this.getUserChoice("Choose a reminder number", true);
  ...
  private handleModifyReminders(): void {
    ...
    if (this._remindersHandler.size() === 0) {
      ...
      Logger.logReminders(this._remindersHandler.reminders);
      const userId = this.getUserChoice("Choose a reminder number", false);
      const description = this.getUserChoice("Choose a reminder description", false);
      this._remindersHandler.modifyReminder(Number(userId) - 1, description);
      this._remindersHandler.toggleCompletion(Number(userId) - 1);
      Logger.log("\n  🏁   Reminder Modified");
    }
  }
  ...
  private handleAddReminder(): void {
    /*
            ...
            ( 4 ) Logger.log('\n  🏁  Reminder Added');
        */
    const typedReminder = this.getUserChoice("Please enter a reminder", false);
    const tag = this.getUserChoice("Please enter a tag", false);
    this._remindersHandler.addReminder(typedReminder, tag);
    Logger.log("\n  🏁  Reminder Added");
  }
  ...
  private handleSearchReminders(): void {
    if (this._remindersHandler.size() === 0) {
      ...
      let keyword = this.getUserChoice("Search", false);
      let result = this._remindersHandler.search(keyword);
      Logger.logSearchResults(result);
    }
    /*
        Pseudocode:
        If no reminders then -> Logger.log('\n  ⚠️  You have no reminders');
